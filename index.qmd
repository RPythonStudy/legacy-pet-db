---
title: Legacy PET Reports Archive
author: BenKorea
...
interpretation_access_raw <- my_read_excel_dir(directory="./data/raw/interpretation_access/", pattern = "\\.xlsx$")
...
pet_interpretation_access_raw[, `ExamDate` := as.Date(as.numeric(`ExamDate`), origin = "1899-12-30")]
...
원자력병원 핵의학과에 PET이 설치된 당시에 판독은 Microsoft사의 Access database에 감마와 PET 판독을 입력하는 시스템을 사용했습니다. Access 상의 최초 PET ExamDate는 `1997-07-16`입니다.
...
pet_interpretation_access[, `ExamDate` := as.Date(as.numeric(`ExamDate`), origin = "1899-12-30")]

pet_interpretation_access_only <- pet_interpretation_access %>%
...
interpretation_ocr_raw <- my_read_excel_dir (directory="./data/raw/interpretation_ocr/", pattern = "\\.xlsx$")
```
