...
readings_access <- readings_access %>%
  filter(
...
```{r}
#| label: readings_access_ptid_validation

readings_access_invalid_ptid <- my_validate_ptid(readings_access, "PtID")
...
target_names <- readings_access_invalid_ptid %>%
  filter(is.na(PtID)) %>%
  pull(PtName) %>%
  unique()
...
# 2. readings_access_invalid_ptid에 업데이트
readings_access_invalid_ptid[
  unique_ptid_dt, 
  on = .(PtName), 
  PtID := i.PtID
...
readings_access[PtName == "민병상", PtID := "814428"]

# colSums(is.na(readings_access))


n_access_only <- nrow(readings_access[ExamDate < as.Date("2000-07-04")])
readings_access[, PtID := my_pad_ptid(PtID)]
...
# ExamDate를 Date 형식으로 변환
readings_ocs[, ExamDate := as.Date(ExamDate)]

matched_readings_access <- readings_access[readings_ocs, 
                                           on = .(PtID, ExamDate), 
                                           nomatch = 0]

readings_merged <- merge(readings_access, readings_ocs,
                    by = c("PtID", "ExamDate"),
                    all = TRUE,
...
preparation_access_invalid_ptid <- my_validate_ptid (preparation_access, "PtID")
colSums(is.na(preparation_access))
